...
const getKeyByValue = (object, param) => {
	object.forEach(el => {
		console.log(Object.values(el));
	});
};

function navigationRender(country, idx) {
...
function cardRender(country_name, hotels) {
	...
	hotels.map((el, idx) => {
		hotels[idx].id = idGenerator();
		item.setAttribute('data-hotel-id', `${el.id}`);
...
function modalGenerator(hotels) {
...
PROMOTION_SETTINGS.map((country, idx) => {
	navigationRender(country, idx);
	promotionsRender(country.name);
	cardRender(country.name, country.hotels);
	// modalGenerator(country.hotels);
});